...
class WarpJSMongoPersistence extends WarpJSPersistence {
    ...
    get _connection() {
        if (!this._db) {
            ...
            return Promise.resolve()
                .then(() => mongodb.MongoClient.connect(url, { useUnifiedTopology: true }))
                .then((client) => {
    ...
    async close(forceClose) {
        if (this._client) {
            return this._client.close(forceClose)
                .then(() => {
                    this._db = null;
                    this._client = null;
                })
    ...
    async documentsLimit(collectionName, limit) {
        return this.collection(collectionName)
            .then((collection) => collection.find().sort({ lastUpdated: -1 }).limit(limit).toArray())
    ...
    async aggregate(collectionName, collectionJoinKey, foreignCollection, foreignKey, aggregateOutputName, matchQuery) {
        const lookup = {
